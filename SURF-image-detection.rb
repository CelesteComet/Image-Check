module FeatureSurfDetection
  ...
  # set test to true if in test environment and would like to see percent similarity.
  TEST_ENV = true

  # set enable GUI to check images
  GUI_ENABLE = false

  class Image

    def self.similar?(img1, img2)
      img1 = CvMat.load(img1, CV_LOAD_IMAGE_GRAYSCALE)
      img2 = CvMat.load(img2, CV_LOAD_IMAGE_GRAYSCALE)
      ...
      if GUI_ENABLE
        GUI::Window.new("#{img1} and #{img2}").show correspond
        GUI::wait_key
      end

      larger_features = [kp1.size, kp2.size].max

      percent_feature_match = match_count / larger_features.to_f

      percent_feature_match_threshold = 0.80

      if TEST_ENV
        puts "Matched by #{percent_feature_match}\n-----------------------------"
      end

      if percent_feature_match >= percent_feature_match_threshold
        true
      else
        false
      end
